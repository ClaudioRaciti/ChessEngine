...
{
    int res = 0;
    const uint64_t whiteSq = 0xaaaaaaaaaaaaaaaa; 
    const uint64_t blackSq = 0x5555555555555555;
    ...
    if(wPawns) do {
        res += (82 + pawnSquareTable[white][btw::bitScanForward(wPawns)]);
    } while (wPawns &= (wPawns - 1)); 
    if(wKnights) do {
        res += (337 + knightSquareTable[white][btw::bitScanForward(wKnights)]);
    } while (wKnights &= (wKnights - 1));
    if(wBishops) do {
        res += (365 + bishopSquareTable[white][btw::bitScanForward(wBishops)]);
    } while (wBishops &= (wBishops - 1));
    if(wRooks) do {
        res += (477 + rookSquareTable[white][btw::bitScanForward(wRooks)]);
    } while (wRooks &= (wRooks - 1));
    if(wQueens) do {
        res += (1025 + queenSquareTable[white][btw::bitScanForward(wQueens)]);
    } while (wQueens &= (wQueens - 1));

    if(bPawns) do {
        res -= (82 + pawnSquareTable[black][btw::bitScanForward(bPawns)]);
    } while (bPawns &= (bPawns - 1)); 
    if(bKnights) do {
        res -= (337 + knightSquareTable[black][btw::bitScanForward(bKnights)]);
    } while (bKnights &= (bKnights - 1));
    if(bBishops) do {
        res -= (365 + bishopSquareTable[black][btw::bitScanForward(bBishops)]);
    } while (bBishops &= (bBishops - 1));
    if(bRooks) do {
        res -= (477 + rookSquareTable[black][btw::bitScanForward(bRooks)]);
    } while (bRooks &= (bRooks - 1));
    if(bQueens) do {
        res -= (1025 + queenSquareTable[black][btw::bitScanForward(bQueens)]);
    } while (wQueens &= (bQueens - 1));

    res += kingSquareTable[white][btw::bitScanForward(wKing)];
    res -= kingSquareTable[black][btw::bitScanForward(bKing)];
